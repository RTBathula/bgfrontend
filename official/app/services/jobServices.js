app.factory('jobServices', function ($http,$rootScope, $q,companyServices) {
    ...
    global.saveJob = function(jobModel){  
     ...
     global.saveJobByObj = function(jobObj){  
        var q = $q.defer();              
    ...
    global.upsertJob = function(jobObj,jobModel){  
        var q = $q.defer(); 
     
        if(!jobObj){
            var jobObj = new CB.CloudObject('Job');
        }
        
        jobObj.set("_user", jobModel._user);
        jobObj.set("_company", jobModel._company);
    ...
    global.searchDeGrande = function(locationObj,
        dateObj,
        gradYear,
        academicGrade,
        degreeName,
